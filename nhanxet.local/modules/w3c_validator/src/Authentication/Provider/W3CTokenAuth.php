...
/**
 * @file
 * Contains \Drupal\w3c_validator\Authentication\Provider\W3CTokenAuth.
 */
...
class W3CTokenAuth implements AuthenticationProviderInterface {
  ...
  public function authenticate(Request $request) {
    ...
    if (!empty($token) && $user = $this->w3cTokenManager->getUserFromToken($token)) {
      ...
      // Log the access.
      $this->logger->notice('Request to validate private page !url using token @token for user @user', array('!url' => $current_url->toString(), '@token' => $token, '@user' => $user->label()));
